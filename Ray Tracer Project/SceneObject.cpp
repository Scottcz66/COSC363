...
glm::vec3 SceneObject::lighting(glm::vec3 lightPos, glm::vec3 lightPos1, glm::vec3 viewVec, glm::vec3 hit)
{
    float ambientTerm = 0.2;
    float diffuseTerm = 0;
    ...
    float lDotn2 = glm::dot(lightVec1, normalVec);
    if (spec_)
    {
        glm::vec3 reflVec1 = glm::reflect(-lightVec, normalVec);
        glm::vec3 reflVec2 = glm::reflect(-lightVec1, normalVec);
    ...
    glm::vec3 colorSum = ambientTerm * color_ + lDotn1 * color_ * 0.4f + specularTerm1 * glm::vec3(1) + 0.4f * lDotn2 * color_ + specularTerm2 * glm::vec3(1);
    return colorSum;
}
