...
//open anti-Aliasing
glm::vec3 antiAliasing(Ray ray, glm::vec3 view, float xIntersection, float yIntersection) {
    float pixel = (XMAX - XMIN) / NUMDIV;
    //make 1 pixel to 4
    Ray quarter1 = Ray(view, glm::vec3(xIntersection + pixel * 0.26, yIntersection + pixel * 0.26, -EDIST));
    quarter1.normalize();
    Ray quarter2 = Ray(view, glm::vec3(xIntersection + pixel * 0.26, yIntersection + pixel * 0.74, -EDIST));
    quarter2.normalize();
    Ray quarter3 = Ray(view, glm::vec3(xIntersection + pixel * 0.74, yIntersection + pixel * 0.26, -EDIST));
    quarter3.normalize();
    Ray quarter4 = Ray(view, glm::vec3(xIntersection + pixel * 0.74, yIntersection + pixel * 0.74, -EDIST));
    quarter4.normalize();
    //set the average number
    glm::vec3 average(0.26);
...
void display()
{
	...
	for (int i = 0; i < NUMDIV; i++)	//Scan every cell of the image plane
	{
		...
		for (int j = 0; j < NUMDIV; j++)
		{
            ...
            //ray.normalize();                                   //Open anti-aliasing
            //glm::vec3 col = antiAliasing(ray, eye, xp, yp);    //Open anti-aliasing
...
void initialize()
{
    ...
    Sphere *sphere1 = new Sphere(glm::vec3(0.0, 9, -90.0), 7.0);    //
    sphere1->setColor(glm::vec3(0.004, 0.188, 0.329));   //Set colour
